...
theorem tri_neg (φ : Prop) : ¬¬¬φ → ¬φ := by
  let h1 : φ → (¬φ → φ) := A7 φ (¬φ)
  let h2 : (¬φ → φ) → ((¬φ → ¬φ) → ¬¬φ) := A9 (¬φ) φ
  have h3 : (φ → (¬φ → φ)) → ((¬φ → φ) → ((¬φ → ¬φ) → ¬¬φ)) → (φ → ((¬φ → ¬φ) → ¬¬φ)) := prop_syl φ (¬φ → φ) ((¬φ → ¬φ) → ¬¬φ)
  have h4 : φ → ((¬φ → ¬φ) → ¬¬φ) := h3 h1 h2
  have h5 : φ → ¬¬φ := by
    intro h6
    have h7 : (¬φ → ¬φ) → ¬¬φ := h4 h6
    have h8 : ¬φ → ¬φ := id_phi (¬φ)
    exact h7 h8
  have h6 : (φ → ¬¬φ) → (¬¬¬φ → ¬φ) := contr_pos φ (¬¬φ)
  exact h6 h5
...
theorem prop_1 (φ ψ γ : Prop) : (γ → ψ) → ((φ → γ) → (φ → ψ)) := by
  intro h1
  intro h2
  have h3 : (φ → γ) → (γ → ψ) → (φ → ψ) := prop_syl φ γ ψ
  exact h3 h2 h1
...
theorem prop_3 (φ γ ψ δ : Prop) : ((((φ ∧ γ) → ψ) → δ) → ((φ → ψ) → δ)) := by
  ...
  intro h2
  let h3 : (φ ∧ γ → φ) := A1 φ γ
  have h4 : (φ ∧ γ → φ) → (φ → ψ) → (φ ∧ γ → ψ) := prop_syl (φ ∧ γ) φ ψ
  have h5 : φ ∧ γ → ψ := h4 h3 h2
  exact h1 h5

theorem prop_4 (φ γ : Prop) (h1 : φ → γ) (h2 : φ → ¬γ) : ¬φ := by
  have h3 : (φ → γ) → ((φ → ¬γ) → ¬φ) := A9 φ γ
  exact h3 h1 h2
